...
SELECT DISTINCT ON (LOWER(email))
    ...
    CASE 
        WHEN TRIM(username) = '' OR username IS NULL THEN NULL
        ELSE TRIM(username) || '_' || ROW_NUMBER() OVER (PARTITION BY LOWER(TRIM(username)) ORDER BY created_at)
    END as username,
...
SELECT DISTINCT ON (LOWER(email))
    ...
    CASE 
        WHEN TRIM(username) = '' OR username IS NULL THEN NULL
        WHEN EXISTS (SELECT 1 FROM bulletproof_users WHERE username = TRIM(users.username)) THEN
            TRIM(users.username) || '_users'
        ELSE TRIM(username)
    END as username,
    NULLIF(TRIM(first_name), '') as first_name,
    password_hash,
...
-- Step 7: Update our clean API to use this table
ALTER TABLE bulletproof_users RENAME TO users_clean;
