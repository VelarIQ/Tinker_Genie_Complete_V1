...
namespace TinkerGenie.API.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        private async Task<string> BuildBurningFiresAIPrompt(string userMessage, string userId)
        {
            ...
            prompt.AppendLine("This is NOT a daily prompt - this is a real problem they need help with RIGHT NOW.");
            prompt.AppendLine();
            prompt.AppendLine($"THEIR URGENT ISSUE: \"{userMessage}\"");
            prompt.AppendLine();
            prompt.AppendLine("YOUR RESPONSE SHOULD:");
            prompt.AppendLine("• Be immediately actionable - they need help TODAY");
            prompt.AppendLine("• Be specific and tactical - not philosophical");
            prompt.AppendLine("• Show urgency and understanding");
            prompt.AppendLine("• Give them 1-2 clear steps they can take right now");
            prompt.AppendLine("• Reference similar situations you've seen with other business owners");
            prompt.AppendLine("• Be supportive but decisive - they're stressed and need direction");
            prompt.AppendLine();
            
            if (userStyle == "short")
            {
                prompt.AppendLine("• ULTRA CONCISE: 3-8 words MAXIMUM total response");
                prompt.AppendLine("• Example: 'Fire drill: Do X immediately.'");
                prompt.AppendLine("• NO explanations - direct action only");
            }
            else if (userStyle == "long")
            {
                prompt.AppendLine("• Provide comprehensive guidance - 5-7 sentences");
                prompt.AppendLine("• Include specific examples and detailed steps");
            }
            else
            {
