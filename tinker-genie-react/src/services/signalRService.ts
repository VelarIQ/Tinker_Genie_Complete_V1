...
import { addMessage, setTyping, setDailyPromptComplete, updateStreamingMessage } from '../store/chatSlice';
...
class SignalRService {
  ...
  private registerHubMethods(): void {
    ...
    // Receive message (full message from backend)
    this.connection.on('ReceiveMessage', (message: any) => {
      // Handle via chatService callback if exists
      if ((window as any).__handleChatResponse) {
        (window as any).__handleChatResponse(message);
      } else {
    ...
    // Receive typing indicator (from backend)
    this.connection.on('ReceiveTypingIndicator', (data: { isTyping: boolean; message?: string }) => {
  ...
  async sendDailyPromptRequest(): Promise<void> {
    if (this.connection?.state !== signalR.HubConnectionState.Connected) {
      throw new Error('Not connected to server');
    }

    await this.connection.invoke('SendDailyPromptRequest');
  }
