...
const ChatPage: React.FC = () => {
  ...
  // Enhanced auto-scroll for messages, typing, and streaming
  useEffect(() => {
    scrollToBottom();
  }, [messages, isTyping, isLoading]);

  // Additional scroll when typing state changes
  useEffect(() => {
    if (isTyping) {
      // Wait a bit for the typing indicator to render
  ...
  // Enhanced scroll function for mobile Safari
  const scrollToBottom = () => {
    // Method 1: scrollIntoView with block end
    messagesEndRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' });
    
    // Method 2: Direct scroll for mobile Safari fallback
    if (messagesContainerRef.current) {
      const container = messagesContainerRef.current;
      // Add extra pixels to ensure absolute bottom
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    // Handle conversation type changes - create new sessions
    if (detection.isExplicitChange) {
      if (detection.type === ConversationType.BURNING_FIRE) {
        ...
        // Store the next message should update title
        (window as any).__pendingConversationType = ConversationType.BURNING_FIRE;
        ...
        // Store the next message should update title
        (window as any).__pendingConversationType = ConversationType.TINKER_LEVEL;
        ...
        inputRef.current?.focus();
        return;
      } else if (detection.type === ConversationType.DAILY_PROMPT) {
        // Start daily prompt session
        await dispatch(startNewChat(ConversationType.DAILY_PROMPT));
        setInputValue('');
        setDictationText('');
        setShowDictationPreview(false);
        return;
  ...
  return (
    <div className="chat-container">
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* Messages Area - with extra bottom padding for mobile */}
        <div 
          ref={messagesContainerRef}
          className="flex-1 overflow-y-auto px-4 py-4"
          style={{ paddingBottom: '120px' }} // Extra padding for mobile keyboard
        >
